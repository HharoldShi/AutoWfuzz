...
public class ProxyServer implements Runnable{
	...
	static Proxy proxy;
	static ProxyDataManipulator dataManipulator ;
	private volatile boolean started = false ;

	// Flags for recording/changing the input/output traffic
	public static  boolean startRecording = false ;
	public static boolean startFuzzing   = false ;
	...
	public ProxyServer(ServerAuthenticator auth){
		this.auth = auth;
		this.dataManipulator = new ProxyDataManipulator() ;
	}
	...
	private void onBind(ProxyMessage msg) throws IOException{
		...
		pipe(in,remote_out, msg.ip.toString() );
	}
	...
	private void startPipe(Socket s){
		...
		try{
			...
			pipe_thread2 = new Thread(this);
			pipe_thread2.start();
			dataManipulator.setDstIP( s.getInetAddress().toString() ) ;
			pipe(in,remote_out, s.getInetAddress().toString() );
		}catch(IOException ioe){
		}
	}
	...
	{
		this.pipe(in, out, null ) ;
	}

	private void pipe(InputStream in,OutputStream out, String ip) throws IOException{
			...
			try{
				if(len!=0){
					if ( startFuzzing == true )
					{
						...
						{
							byte[] reply =  dataManipulator.modifyMsg( buf, len, ip ) ;
							boolean sameReply = true ;
							...
							int counter = 0 ;
							while ( counter < reply.length && reply[counter] != 0 )
								counter++ ;
							if ( sameReply == true )
								out.write( buf, 0, len ) ;
							else
								out.write( reply, 0, counter ) ;
						}
					}
					else if ( startRecording == true )
					{
						dataManipulator.recordMsg( buf, BUF_SIZE, ip ) ;
						out.write(buf,0,len);
					}
					else
						out.write(buf,0,len);
	 ...
	 * 
	 * @return True if the recording is successfully switched on
	 */
	 ...
	 * 
	 * @return True is the recording is successfully switched off
	 */
