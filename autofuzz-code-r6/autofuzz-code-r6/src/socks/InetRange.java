...
public class InetRange implements Cloneable{
    ...
    public synchronized boolean remove(String s){
      Enumeration enumq = all.elements();
      while(enumq.hasMoreElements()){
        Object[] entry = (Object[]) enumq.nextElement();
        if(s.equals(entry[0])){
...
/////////////////
    ...
    private synchronized boolean contains(long ip){
       Enumeration enumq = all.elements();
       while(enumq.hasMoreElements()){
         Object[] obj = (Object[]) enumq.nextElement();
    ...
    private boolean checkHostEnding(String host){
       Enumeration enumq = end_names.elements();
       while(enumq.hasMoreElements()){
          if(host.endsWith((String) enumq.nextElement())) return true;
       }
       return false;
    }
