...
//////////////////
  private int getRangeIndex(InetAddress ip){
     int index = 0;
     Enumeration enumq = hosts.elements();
     while(enumq.hasMoreElements()){
       InetRange ir = (InetRange) enumq.nextElement();
  ...
  private String userNames(int i){
    ...
    Enumeration enumq = ((Hashtable)users.elementAt(i)).keys();
    if(!enumq.hasMoreElements()) return "";
    String s = enumq.nextElement().toString();
    while(enumq.hasMoreElements())
       s += "; "+enumq.nextElement();
